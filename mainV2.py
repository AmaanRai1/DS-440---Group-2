...
# Example usage
def main(index_name, time_window=6):
    # Step 1: Load the data
    df = load_data_from_github(index_name)

    # Step 2: Split the data into train, validation, and test sets
    train_data_final, validation_data, test_data = split_data(df)

    # Step 3: Create train, validation, and test sequences
    ...
    print("Input sequences (X_train) shape:", x_train.shape)
    print("Output targets (y_train) shape:", y_train.shape)
    print("Validation sequences (X_val) shape:", x_val.shape)
    print("Validation targets (y_val) shape:", y_val.shape)
    print("Test sequences (X_test) shape:", x_test.shape)
    print("Test targets (y_test) shape:", y_test.shape)

    return x_train, y_train, x_val, y_val, x_test, y_test

# If running this script directly, test with an example
if __name__ == "__main__":
    # Load DJI data and split into sequences
    time_window = 6  # Adjust as needed
    x_train, y_train, x_val, y_val, x_test, y_test = main("DJI", time_window)
