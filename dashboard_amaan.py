...
#         subprocess.run(["git", "clone", repo_url], check=True, text=True)
...
# Verify cloning
# st.write("Files in repository:", os.listdir(repo_dir))

# repo_dir = "DS-440---Group-2"
# main_file = os.path.join(repo_dir, "mainV2_saiem.py")

# if os.path.exists(main_file):
#     st.write(f"Found {main_file}")
# else:
#     st.error(f"{main_file} not found. Verify the repository structure.")

# Add the repo directory to the system path
sys.path.append(os.path.abspath(repo_dir))

try:
    from mainV2_saiem import *  # Adjust this based on what you need from the file
    st.write("Successfully imported `mainV2_saiem.py`!")
except ImportError as e:
    st.error(f"Error importing mainV2_saiem: {e}")
